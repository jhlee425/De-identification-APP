...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        if self.mode == 'video':
            progress_bar = ProgressBar(30, self.nframes)
        if self.video_flag[self.count]:
            ...
            progress_bar = ProgressBar(30, self.nframes)
            self.frame += 1
            progress = self.frame / self.nframes * 100
            progress_bar.set_val(self.frame)
            print('\rProcessing Frame %s %g / %g (%5.2g%%)' % (repr(progress_bar), self.frame + 1, self.nframes, progress), end='')
